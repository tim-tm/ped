...
int main(int argc, char **argv) {
  ...
  WINDOW *text_win = newwin(state.max_y - infobar_height,
                            state.max_x - state.line_size, 0, state.line_size);
  WINDOW *infobar_win =
  ...
  while (c != CTRL('q') && !close_requested) {
    ...
    for (size_t i = 0; i < buf.size; ++i) {
      ...
      mvwprintw(line_win, i - buf.scroll_y, state.line_size - 2 - l_size, "%zu",
                i + 1);
      if (itr->size != 0) {
